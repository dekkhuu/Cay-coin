...
# One Piece faction roles
PIRATE_ROLE_ID = 1552877523140878396
MARINE_ROLE_ID = 1552877844575686806


ALLOWED_GUILD_ID = 1503922700408586240

ONEPIECE_SHOP_CHANNEL_NAME = "🍎・shop-trai-ac-quy"
ONEPIECE_SHOP_MESSAGES = {}
...
def get_gacha_single_cost(player):
    """Giá random theo level: Lv.1 = 10.000, Lv.2 = 20.000; sau đó mỗi 10 level tăng 10.000."""
    level = max(1, int(player.get("level", 1)))
    if level == 1:
        return 10_000
    return 20_000 + ((level - 2) // 10) * 10_000
...
def _island_scope_key(sea, island_key):
    return f"{int(sea)}:{island_key}"

def ensure_island_state(player):
    """Keep each Sea/Island navigation and quest progress isolated."""
    current_sea = int(player.get("current_sea", 1) or 1)
    current_island = player.get("current_island")

    current_map = player.get("current_islands")
    if not isinstance(current_map, dict):
        current_map = {}
    # Migrate the old single current_island value into the correct Sea.
    if current_island:
        current_map.setdefault(str(current_sea), current_island)
    player["current_islands"] = current_map

    progress_map = player.get("quest_progress_by_island")
    if not isinstance(progress_map, dict):
        progress_map = {}
    # Migrate old shared progress into the currently selected island.
    old_progress = int(player.get("quest_progress", 0) or 0)
    if current_island:
        progress_map.setdefault(_island_scope_key(current_sea, current_island), old_progress)
    player["quest_progress_by_island"] = progress_map

    active_key = current_map.get(str(current_sea))
    if active_key not in SEA_DATA.get(current_sea, {}).get("islands", {}):
        active_key = next(iter(SEA_DATA[current_sea]["islands"]), None)
        if active_key:
            current_map[str(current_sea)] = active_key
    player["current_sea"] = current_sea
    player["current_island"] = active_key
    player["quest_progress"] = int(progress_map.get(_island_scope_key(current_sea, active_key), 0)) if active_key else 0
    return player

def get_current_island(player, sea=None):
    ensure_island_state(player)
    sea = int(sea if sea is not None else player.get("current_sea", 1))
    key = player.get("current_islands", {}).get(str(sea))
    if key in SEA_DATA[sea]["islands"]:
        return key
    return next(iter(SEA_DATA[sea]["islands"]), None)

def set_current_island(player, sea, island_key):
    ensure_island_state(player)
    sea = int(sea)
    if island_key not in SEA_DATA[sea]["islands"]:
        raise KeyError(f"Đảo '{island_key}' không thuộc Sea {sea}")
    player["current_sea"] = sea
    player.setdefault("current_islands", {})[str(sea)] = island_key
    player["current_island"] = island_key
    player["quest_progress"] = int(player.setdefault("quest_progress_by_island", {}).get(_island_scope_key(sea, island_key), 0))

def get_quest_progress(player, sea, island_key):
    ensure_island_state(player)
    key = _island_scope_key(sea, island_key)
    return int(player.setdefault("quest_progress_by_island", {}).get(key, 0))

def set_quest_progress(player, sea, island_key, value):
    ensure_island_state(player)
    value = max(0, int(value))
    player.setdefault("quest_progress_by_island", {})[_island_scope_key(sea, island_key)] = value
    if int(player.get("current_sea", 1)) == int(sea) and player.get("current_island") == island_key:
...
def create_player(faction):
    return {
        ...
        "current_sea": 1, "current_island": None, "current_islands": {"1": "starter_island", "2": None, "3": None},
        "quest_progress": 0, "quest_progress_by_island": {}, "quest_weapon": None,
...
def simulate_fight(player, sea, island_key, weapon, skill_index=0):
    ...
    if win:
        ...
        set_quest_progress(player, sea, island_key, get_quest_progress(player, sea, island_key) + 1)
...
def build_onepiece_help_embed():
    return discord.Embed(
        ...
        description=(
            ...
            "🍎 **Sell** — Hệ thống bán trái / Dealer\n"
            "🛒 **Shop** — Kênh riêng, trái đổi mỗi 1 giờ\n"
...
def build_farm_embed(player):
    ensure_island_state(player)
    sea = int(player.get("current_sea", 1))
    ik = get_current_island(player, sea)
    island_line = ""
    if ik:
        try:
            island = SEA_DATA[sea]["islands"].get(ik)
            if island:
                progress = get_quest_progress(player, sea, ik)
...
async def setup_faction_access(guild: discord.Guild, member: discord.Member, faction: str):
    """Cấp đúng role phe và khóa 2 kênh phe theo role."""
    pirate_role = guild.get_role(PIRATE_ROLE_ID)
    marine_role = guild.get_role(MARINE_ROLE_ID)
    if pirate_role is None or marine_role is None:
        raise RuntimeError("Không tìm thấy role Hải Tặc/Hải Quân theo ID đã cấu hình.")

    is_pirate = "Hải Tặc" in faction
    wanted_role = pirate_role if is_pirate else marine_role
    other_role = marine_role if is_pirate else pirate_role

    # Cấp role phe đã chọn, bỏ role phe còn lại.
    if other_role in member.roles:
        await member.remove_roles(other_role, reason="One Piece đổi phe")
    if wanted_role not in member.roles:
        await member.add_roles(wanted_role, reason="One Piece chọn phe")

    channels = {
        "🏴‍☠️-hai-tac": pirate_role,
        "⚓-hai-quan": marine_role,
    }
    for name, role in channels.items():
        channel = discord.utils.get(guild.text_channels, name=name)
        if channel is None:
            overwrites = {
                guild.default_role: discord.PermissionOverwrite(view_channel=False),
                pirate_role: discord.PermissionOverwrite(view_channel=(role == pirate_role), send_messages=(role == pirate_role), read_message_history=(role == pirate_role)),
                marine_role: discord.PermissionOverwrite(view_channel=(role == marine_role), send_messages=(role == marine_role), read_message_history=(role == marine_role)),
            }
            ...
            channel = await guild.create_text_channel(name, overwrites=overwrites, reason="One Piece faction channels")
        else:
            await channel.set_permissions(guild.default_role, view_channel=False, send_messages=False, read_message_history=False)
            await channel.set_permissions(pirate_role, view_channel=(role == pirate_role), send_messages=(role == pirate_role), read_message_history=(role == pirate_role))
            await channel.set_permissions(marine_role, view_channel=(role == marine_role), send_messages=(role == marine_role), read_message_history=(role == marine_role))
            if guild.me is not None:
                await channel.set_permissions(guild.me, view_channel=True, send_messages=True, read_message_history=True, manage_channels=True)
...
class ChooseFactionView(discord.ui.View):
    ...
    async def _pick(self, interaction, faction):
        ...
        if user.id in player_data:
            ...
            try:
                await setup_faction_access(interaction.guild, user, faction)
        ...
        try:
            ...
            player_data.pop(user.id, None)
            await interaction.response.send_message(f"❌ Không thể cấp role/tạo quyền kênh phe: {e}", ephemeral=True)
            return
...
class FarmView(discord.ui.View):
    ...
    async def quest(self, interaction, button):
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("⚠️ Không phải bạn!", ephemeral=True); return
        p = ensure_island_state(player_data[self.user_id])
        sea = int(p.get("current_sea", 1))
        ik = get_current_island(p, sea)
        set_current_island(p, sea, ik)
        await interaction.response.edit_message(
...
def build_island_list_embed(player, sea=None):
    ...
    current = get_current_island(player, sea)
    lines = []
...
def build_island_quest_embed(player, sea, island_key):
    ...
    progress = get_quest_progress(player, sea, island_key)
    need = isl["quest_kill"]
...
class IslandListView(discord.ui.View):
    def __init__(self, user_id, sea=None):
        ...
        current = get_current_island(p, self.sea)
    ...
    def _pick(self, ik):
        async def cb(i):
            if i.user.id != self.user_id:
                await i.response.send_message("⚠️ Không phải bạn!", ephemeral=True); return
            p = ensure_island_state(player_data[self.user_id])
            set_current_island(p, self.sea, ik)
    ...
    async def _prev(self, i):
        ...
        if self.sea > 1:
            p = ensure_island_state(player_data[self.user_id])
            target_sea = self.sea - 1
            set_current_island(p, target_sea, get_current_island(p, target_sea))
            await i.response.edit_message(embed=build_island_list_embed(p, target_sea),
...
class IslandQuestView(discord.ui.View):
    ...
    async def boss(self, i, b):
        ...
        if get_quest_progress(p, self.sea, self.island_key) < isl["quest_kill"]:
        ...
        async def cb(i):
            ...
            em = discord.Embed(
                ...
                description=(
                    ...
                    f"\n📊 Lv.`{p['level']}` | 🎯 `{get_quest_progress(p, self.sea, self.island_key)}/{isl['quest_kill']}`"),
                color=0x2ECC71 if res['win'] else 0xE74C3C)
            view = QuestCompleteView(self.user_id, self.sea, self.island_key) if get_quest_progress(p, self.sea, self.island_key) >= isl["quest_kill"] else self
...
class QuestCompleteView(discord.ui.View):
    ...
    async def claim(self, i, b):
        ...
        set_quest_progress(p, self.sea, self.island_key, 0)
...
class GachaView(discord.ui.View):
    ...
    async def _roll(self, i, count, cost_key):
        if i.user.id != self.user_id:
            await i.response.send_message("⚠️ Không phải bạn!", ephemeral=True); return
        p = player_data[self.user_id]
        free_first = count == 1 and not p.get("free_random_used", False)
        cost = 0 if free_first else get_gacha_cost(p, count)
        if not free_first and p["coin"] < cost:
            await i.response.send_message(f"❌ Cần `{cost:,}` Coin! (Lv.{p.get('level',1)})", ephemeral=True); return
        if free_first:
            p["free_random_used"] = True
        else:
            p["coin"] -= cost
        results = []
        ...
        em = build_gacha_result_embed(results, p, new, is_multi=(count > 1), free=free_first)
    ...
    async def buy(self, interaction: discord.Interaction, button):
        try:
            ...
            if not player:
                ...
                return
            player = normalize_player_loadout(player)
            await interaction.response.send_message(
                embed=build_shop_embed(player, "fruits"),
                view=ShopView(interaction.user.id, "fruits"),
                ephemeral=True,
            )
        except (discord.NotFound, discord.Forbidden, discord.HTTPException) as e:
            print(f"[SHOP BUTTON] Discord error: {type(e).__name__}: {e}")
        except Exception as e:
            print(f"[SHOP BUTTON] Unexpected error: {type(e).__name__}: {e}")

async def ensure_onepiece_shop_channel(guild: discord.Guild):
    if guild is None:
        return None

    # Tìm kênh cũ trước để không tạo trùng.
    channel = discord.utils.find(
        lambda c: isinstance(c, discord.TextChannel)
        and c.name.lower() == ONEPIECE_SHOP_CHANNEL_NAME.lower(),
        guild.text_channels,
    )
    ...
    if channel is None:
        try:
            channel = await guild.create_text_channel(
                ONEPIECE_SHOP_CHANNEL_NAME,
                reason="One Piece Shop riêng — stock đổi mỗi 1 giờ",
            )
        except (discord.Forbidden, discord.HTTPException) as e:
            print(f"[SHOP] Không thể tạo kênh trong guild {guild.id}: {type(e).__name__}: {e}")
            return None

    # Cập nhật quyền cho bot nếu Discord cache cho phép.
    try:
        me = guild.me or guild.get_member(bot.user.id if bot.user else 0)
        if me is not None:
            await channel.set_permissions(
                me, view_channel=True, send_messages=True,
                embed_links=True, read_message_history=True,
            )
    except (discord.Forbidden, discord.HTTPException):
        pass

    # Tìm message shop do bot gửi, không tạo thêm nếu đã có.
    message = None
    cached_id = ONEPIECE_SHOP_MESSAGES.get(guild.id)
    if cached_id:
        try:
    ...
    embed = build_hourly_dealer_embed(None)
    embed.title = "🍎 Shop Trái Ác Quỷ • BirthdayTime"
    embed.set_footer(text="Kho trái tự động đổi sau mỗi 1 giờ • Bấm nút để mua")

    try:
        if message is None:
            message = await channel.send(embed=embed, view=PublicShopView())
        else:
            await message.edit(embed=embed, view=PublicShopView())
        ONEPIECE_SHOP_MESSAGES[guild.id] = message.id
    except (discord.Forbidden, discord.HTTPException) as e:
        print(f"[SHOP] Không thể gửi/cập nhật embed guild {guild.id}: {type(e).__name__}: {e}")
        return None
    return channel

@tasks.loop(seconds=60)
async def refresh_onepiece_shop_channel():
    guilds = [g for g in list(bot.guilds) if g.id == ALLOWED_GUILD_ID]
    for guild in guilds:
        try:
            await ensure_onepiece_shop_channel(guild)
        except Exception as e:
            print(f"[SHOP REFRESH] Guild {guild.id}: {type(e).__name__}: {e}")

@refresh_onepiece_shop_channel.before_loop
async def before_refresh_onepiece_shop_channel():
    await bot.wait_until_ready()
...
async def cmd_onepiece(interaction: discord.Interaction):
    try:
        ...
        print(f"[SHOP SETUP] {type(e).__name__}: {e}")
