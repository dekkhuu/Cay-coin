...
class BauCuaBetModal(discord.ui.Modal, title='Đặt cược Bầu Cua'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Trừ coin đối với thành viên thường; SPECIAL_ADMIN_ID vẫn là tài khoản vô hạn.
        if not infinite:
...
async def thong_bao_orb(ten_nhiem_vu: str, mo_ta: str, reward: str='Orb', quest_id: str='Chưa có', thoi_gian_ket_thuc: str='Chưa có', quest_url: str | None=None):
    """Chỉ gửi khi có Quest hợp lệ; không tự tạo thông báo nếu API không có Quest."""
...
def _quest_is_active(q):
    """Chỉ nhận Quest đang hoạt động; loại Quest đã hết hạn/chưa mở."""
...
async def scan_public_quests(notify_new=True):
    """Quét Quest nhưng chống gửi trùng khi scanner và /scanorb chạy cùng lúc."""
    ...
    async with QUEST_SCAN_LOCK:
        ...
        # Lần quét đầu chỉ tạo mốc, tuyệt đối không gửi hàng loạt Quest đang có.
        ...
        # Chỉ ID chưa từng thấy mới được xem là Quest mới.
        new_quests = []
        batch_ids = set()
        for q in quests:
            qid = str(q['id'])
        ...
        for q in new_quests:
            if notify_new:
                ...
                # Chỉ đánh dấu đã thông báo sau khi Discord nhận message thành công.
                if sent:
