...
# ============================================================
# GAME 9 Ô — CHỌN Ô AN TOÀN
# 9 ô, mỗi ô có một nút. Mỗi ván có 1 ô X (thua), 8 ô ✓ (an toàn).
# Khi người chơi bấm ô:
#   ✓ = an toàn, ô được khóa.
#   X = thua, toàn bộ bàn chơi kết thúc.
# ============================================================
NINEBOX_GAMES = {}

class NineBoxButton(discord.ui.Button):
    def __init__(self, game_id: str, index: int):
        super().__init__(
            label=str(index + 1),
            style=discord.ButtonStyle.secondary,
            custom_id=f"ninebox_{game_id}_{index}",
            row=index // 3,
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        game = NINEBOX_GAMES.get(self.game_id)

        if not game or game.get("finished"):
            return await interaction.response.send_message(
                '<a:failed:1548973085741547580> Ván 9 ô đã kết thúc.',
                ephemeral=True
            )

        if interaction.channel_id != game["channel_id"]:
            return await interaction.response.send_message(
                '<a:failed:1548973085741547580> Đây không phải bàn chơi này.',
                ephemeral=True
            )

        # Không cho một người bấm lại cùng một ô.
        if self.index in game["revealed"]:
            return await interaction.response.send_message(
                '<a:failed:1548973085741547580> Ô này đã được mở rồi.',
                ephemeral=True
            )
        ...
        if self.index == game["mine"]:
            ...
            # Hiện toàn bộ kết quả khi có người chạm ô X.
            ...
            # Người dính mìn nhận 500 coin khi ván kết thúc.
            baucua_change_coins(interaction.user.id, 500)

            embed = make_embed(
                title="💥 GAME MÌN • KẾT THÚC",
                description=(
                    "**Chọn các ô dưới và hãy né mìn nha!**\n\n"
                    f"{interaction.user.mention} đã chọn ô `{self.index + 1}` kết quả: `❌ dính mìn`\n\n"
                    "💥 **Ván chơi kết thúc!**\n"
                    f"🪙 {interaction.user.mention} nhận được **500 coin**!\n"
                    f"💣 Ô mìn là **ô {game['mine'] + 1}**."
                ),
        ...
        embed = make_embed(
            ...
            description=(
                "**Chọn các ô dưới và hãy né mìn nha!**\n\n"
                f"Tới lượt của {interaction.user.mention} hãy chọn ô để né mìn\n\n"
                f"{interaction.user.mention} đã chọn ô `{self.index + 1}` kết quả: `✓ an toàn`\n"
                f"\nĐã mở **{safe_count}/8 ô an toàn**."
            ),
            color=discord.Color.green(),
        )
        await interaction.response.edit_message(
...
class NineBoxView(discord.ui.View):
    def __init__(self, game_id: str):
        super().__init__(timeout=None)
        self.game_id = game_id
        for index in range(9):
            self.add_item(NineBoxButton(game_id, index))


@bot.tree.command(name="min", description="Chơi game Mìn 9 ô")
async def nine_box_command(interaction: discord.Interaction):
    if interaction.guild is None:
        return await interaction.response.send_message(
            '<a:failed:1548973085741547580> Game 9 ô chỉ dùng trong server.',
            ephemeral=True
        )

    # Mỗi kênh chỉ có một bàn đang hoạt động.
    for game in NINEBOX_GAMES.values():
        if (
            ...
            return await interaction.response.send_message(
                '<a:failed:1548973085741547580> Kênh này đang có một ván 9 ô. Hãy chơi ván hiện tại trước.',
                ephemeral=True
            )

    game_id = f"ninebox_{interaction.guild_id}_{interaction.channel_id}_{interaction.id}"
    view = NineBoxView(game_id)

    NINEBOX_GAMES[game_id] = {
    ...
    embed = make_embed(
        ...
        description=(
            "**Chọn các ô dưới và hãy né mìn nha!**\n\n"
            f"Tới lượt của {interaction.user.mention} hãy chọn ô để né mìn\n\n"
            "Chọn một trong các ô **1 → 9** bên dưới."
        ),
