...
NOITU_GAMES = {}
NOITU_RECENT_GAMES = []
NOITU_RECENT_LIMIT = 10
NOITU_TURN_TIMEOUT = 30
NOITU_ERROR_TIMEOUT = 10
...
def noitu_find_reply(required: str, used: set[str]) -> str | None:
    ...
    candidates = [
        ...
        if w.lower().split()[0] == required
        and w.lower() not in used
        and w.lower() not in {x.lower() for game_words in NOITU_RECENT_GAMES for x in game_words}
    ]
...
async def noitu_error(channel, text: str):
    ...
    await asyncio.sleep(NOITU_ERROR_TIMEOUT)
    try:
        await msg.delete()
...
async def noitu_send_turn(channel, prefix: str, word: str, required: str):
    embed = noitu_embed(
        "Nối từ • BirthdayTime",
        f"{prefix}\n"
        f"Bạn hãy nối tiếp với từ: **{required}**"
    )
...
async def noitu_finish(channel, game, winner_id: int):
    if game.get("finished"):
        return
    game["finished"] = True
    finished_words = set(game.get("current_game_words", set()))
    if finished_words:
        NOITU_RECENT_GAMES.append(finished_words)
        del NOITU_RECENT_GAMES[:-NOITU_RECENT_LIMIT]
    reward = baucua_change_coins(winner_id, 1000)
    embed = noitu_embed(
        "Nối từ • BirthdayTime",
...
async def noitu_bot_turn(channel, game):
    if game.get("finished") or not game.get("started"):
        return

    bot_reply = noitu_find_reply(
        game["required"],
        game["used"],
    )

    if bot_reply is None:
        # Nếu bot không thể nối thì kết thúc ván và người chơi cuối cùng thắng.
        winner_id = game.get("last_player_id")
        if winner_id:
            await noitu_finish(channel, game, winner_id)
        return

    game["used"].add(bot_reply.lower())
    game["current_game_words"].add(bot_reply.lower())
    game["bot_word"] = bot_reply
    game["required"] = noitu_required_word(bot_reply)
    game["turn_message"] = await noitu_send_turn(
        channel,
        f"Bot đã nối tiếp với từ: **{bot_reply}**",
        bot_reply,
        game["required"],
    )
    game["turn_deadline"] = asyncio.get_running_loop().time() + NOITU_TURN_TIMEOUT
    game["timer_task"] = asyncio.create_task(noitu_turn_timer(channel, game))


async def noitu_turn_timer(channel, game):
    try:
        await asyncio.sleep(NOITU_TURN_TIMEOUT)
        if game.get("finished") or not game.get("started"):
            return
        if game.get("timer_task") is not asyncio.current_task():
            return
        await noitu_bot_turn(channel, game)
    except asyncio.CancelledError:
        return


async def noitu_start(channel: discord.TextChannel):
    ...
    start = noitu_find_reply("", set())
    if start is None:
        candidates = [w for w in NOITU_WORDS if w.lower() not in {x.lower() for game_words in NOITU_RECENT_GAMES for x in game_words}]
        if not candidates:
            candidates = NOITU_WORDS
        if not candidates:
            await noitu_error(channel, "Không có dữ liệu từ để bắt đầu")
            return False
        start = random.choice(candidates)

    game["current_game_words"] = {start.lower()}
    game.update({
        "started": True,
        "finished": False,
        "bot_word": start,
...
async def on_message(message: discord.Message):
    ...
    if game and game.get("started") and not game.get("finished"):
        ...
        # Nếu đã có người nối ở lượt hiện tại thì tin nhắn khác không hợp lệ.
        # Việc xác định lượt theo tin nhắn cuối cùng: chỉ người đầu tiên gửi câu hợp lệ được tính.
        if game.get("processing_turn") or message.author.id == game.get("last_player_id"):
        ...
        if user_word in game["used"] or user_word in {x.lower() for game_words in NOITU_RECENT_GAMES for x in game_words}:
        ...
        game["current_game_words"].add(user_word)
        game["last_player_id"] = message.author.id
        game["player_correct"] = game.get("player_correct", 0) + 1
        baucua_change_coins(message.author.id, 100)

        bot_reply = noitu_find_reply(
            noitu_required_word(user_word),
            game["used"],
        )
        ...
        game["turn_message"] = await noitu_send_turn(
            message.channel,
            f"{message.author.mention} đã ra từ: **{user_word}**",
            user_word,
            game["required"],
        )
        game["timer_task"] = asyncio.create_task(noitu_turn_timer(message.channel, game))
        return

    await bot.process_commands(message)


# Đồng bộ slash command khi bot sẵn sàng.
@bot.event
async def on_ready():
