...
async def noitu_finish(channel, game, winner_id: int):
    ...
    await channel.send(
        f"🏆 <@{winner_id}> đã chiến thắng Nối từ!\n"
        f"🪙 Nhận được **1000 coin**!\n"
        f"💰 Số dư: **{reward} coin**"
    )
...
async def noitu_command(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title="Nối từ • BirthdayTime",
        description="Hãy nhập lệnh `!start` để Bot bắt đầu nối từ",
        color=discord.Color.blurple(),
    )
    await interaction.response.send_message(embed=embed)


async def noitu_start(channel: discord.TextChannel, starter: discord.Member | discord.User):
    ...
    # Sau khi !start, các lượt chơi được trả lời bằng tin nhắn thường, không embed.
    await channel.send(
        f"<@{starter.id}> ra từ: {start}\n"
        f"Bạn từ bạn cần nối là: {required}"
    )
    return True
...
async def on_message(message: discord.Message):
    ...
    # !start bắt đầu ván đã tạo bằng /noitu.
    if message.content.strip().lower() == "!start":
        if not game or game.get("finished"):
            await message.channel.send(
                "<a:failed:1548973085741547580> Hãy dùng `/noitu` trước để tạo ván."
            )
        elif game.get("started"):
            await message.channel.send(
                "<a:failed:1548973085741547580> Ván Nối từ đã bắt đầu rồi!"
            )
        elif message.author.id != game.get("owner_id"):
            await message.channel.send(
                "<a:failed:1548973085741547580> Chỉ người tạo phòng mới được dùng `!start`."
            )
        else:
            await noitu_start(message.channel, message.author)
        return

    # Sau !start, người chơi chỉ cần chat từ/cụm từ, không cần lệnh.
    if game and game.get("started") and not game.get("finished"):
        if message.author.id != game.get("player_id"):
            return
        ...
        if len(parts) < 2:
            ...
            invalid = True
        elif user_word in game["used"]:
            invalid = True

        if invalid:
            bad = await message.channel.send(
                "<a:failed:1548973085741547580> Từ của bạn không hợp lệ"
            )
            # Giữ thông báo 15 giây rồi xóa.
        ...
        # Mỗi câu đúng của người chơi +100 coin.
        game["used"].add(user_word)
        game["player_correct"] += 1
        new_balance = baucua_change_coins(message.author.id, 100)
        ...
        # Nếu bot không còn từ để nối, người chơi thắng +1000 coin.
        ...
        # Trả lời bằng chat thường, không embed.
        await message.channel.send(
            f"<@{message.author.id}> ra từ: {bot_reply}\n"
            f"Bạn từ bạn cần nối là: {game['required']}"
        )
        return

    # Giữ cho các command prefix khác của bot hoạt động bình thường.
